...
function buildBundleScript(moduleFactories, bootstrapCode) {
  return [
    ...
    '    // En el preview, BrowserRouter falla porque el iframe sandbox tiene origen opaco',
    '    // (sin allow-same-origin), lo que bloquea window.history.pushState.',
    '    // Sustituimos BrowserRouter por MemoryRouter solo para el preview.',
    '    // El código fuente del proyecto del usuario no se modifica.',
    '    return new Proxy(rrd, {',
    '      get: function(target, prop) {',
    '        if (prop === "BrowserRouter") return target.MemoryRouter;',
    '        if (prop === "HashRouter") return target.MemoryRouter;',
    '        return target[prop];',
    '      }',
    '    });',
    '  }',
