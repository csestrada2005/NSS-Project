...
// FUENTE. Este archivo combina esa auditoría con una prueba funcional real
// de las dos piezas puras que la línea conecta:
//   1. el fallback importa y usa promptNeedsServer(input), no `false` fijo
//      (si alguien revierte la línea, esto lo detecta);
...
test('AIOrchestrator.ts importa promptNeedsServer de serverLogicSignals.js', () => {
  assert.match(
    ORCHESTRATOR_SRC,
...
test('el fallback de "No memory available" ya no fija needs_server en false', () => {
...
test('con memory null y un prompt que dispara el cinturón, el intent trae needs_server:true', () => {
  const intent = buildNoMemoryIntent('quiero poder invitar usuarios y asignarles un rol de administrador');
  assert.equal(intent.needs_server, true);
  assert.equal(intent.server_reason, 'deterministic signal');
});

test('y por tanto isPlanLaneOnly manda ese intent al plan lane', () => {
  const intent = buildNoMemoryIntent('quiero poder invitar usuarios y asignarles un rol de administrador');
  assert.equal(isPlanLaneOnly(intent), true);
});

test('un prompt que no dispara el cinturón deja needs_server:false, sin cambiar el comportamiento previo', () => {
  const intent = buildNoMemoryIntent('cambia el color principal del sitio');
  assert.equal(intent.needs_server, false);
  assert.equal(intent.server_reason, '');
});
