/**
 * shadcn/ui base components shipped in every generated project (CIRUGÍA P1-6,
 ...
 *
 * Radix + cva versions are PINNED in UI_COMPONENT_DEPENDENCIES (CAMBIO 4) and
 * kept in lockstep with AIOrchestrator's KNOWN_DEP_VERSIONS so an exported
 * `npm install && vite build` reproduces the same tree the preview runs.
 */
...
// --- accordion ------------------------------------------------------------
const ACCORDION = `import * as React from "react";
...
export const UI_DIR = {
  directory: {
    'button.tsx': file(BUTTON),
    'card.tsx': file(CARD),
    'input.tsx': file(INPUT),
    'textarea.tsx': file(TEXTAREA),
    'label.tsx': file(LABEL),
    'badge.tsx': file(BADGE),
    'separator.tsx': file(SEPARATOR),
    'accordion.tsx': file(ACCORDION),
    'tabs.tsx': file(TABS),
    'dialog.tsx': file(DIALOG),
  },
};

/**
 * Extra runtime dependencies the 10 shadcn components import, PINNED to current
 * React-18-compatible versions (CAMBIO 4). Merged into the template package.json
 * on top of SHADCN_DEPENDENCIES. Radix resolves via esm.sh in the preview and
 * via a real install in an exported project — keep in lockstep with
 * AIOrchestrator's KNOWN_DEP_VERSIONS.
 */
export const UI_COMPONENT_DEPENDENCIES = {
