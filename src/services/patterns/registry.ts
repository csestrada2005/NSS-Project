...
export const PATTERN_DATA: InjectionPattern[] = [
  ...
  },
  {
    ...
    "rules": [
      "For plain viewport reveals, PREFER the template's <FadeIn>/<StaggerChildren> from '@/components/motion' over a bespoke MotionReveal; reserve this pattern's custom useScrollState hook for scroll-progress-synchronized effects (parallax, progress bars) the primitives don't provide, throttling its window scroll events with requestAnimationFrame and a ref-based guard to prevent layout thrashing.",
  ...
  },
  {
    ...
    "rules": [
      "PREFER the template's shipped primitives: import { FadeIn, StaggerChildren } from '@/components/motion' and wrap content in <FadeIn> (with <StaggerChildren> for sequential lists) instead of hand-rolling this IntersectionObserver Reveal. Only reimplement the observer below when you must avoid the framer-motion dependency entirely; if you do, evaluate window.matchMedia('(prefers-reduced-motion: reduce)').matches synchronously before creating the observer and set visibility to true for reduced-motion users.",
  ...
  },
  {
    ...
    "rules": [
      "FIRST compose the template's motion primitives: import { FadeIn, StaggerChildren } from '@/components/motion' and wrap content in <FadeIn direction=\"up\" delay={0.1}> for standard entrance/scroll reveals (grouping sequential items in <StaggerChildren>). They are viewport-triggered and respect prefers-reduced-motion. Do NOT hand-write initial/whileInView variants for a plain reveal — compose the primitive.",
      "Drop to raw framer-motion (import { motion } from 'framer-motion') ONLY for motion the primitives do not cover: bespoke hero orchestration, AnimatePresence exit transitions, drag, or layout animations. framer-motion is vendored in the preview — never fall back to CSS transitions or requestAnimationFrame when the user asks for animation by name.",
      "Use initial/animate for entrance, whileHover/whileTap for interaction, AnimatePresence for exit; keep entrance durations 0.4–0.8s with ease-out."
    ],
    "maxTokenEstimate": 420,
    "incompatibleWith": []
  }
